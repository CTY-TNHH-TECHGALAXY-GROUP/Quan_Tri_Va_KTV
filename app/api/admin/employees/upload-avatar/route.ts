...
export async function POST(request: Request) {
  try {
    ...
    if (!(file instanceof File) || typeof staffId !== 'string' || !staffId.trim()) {
      return NextResponse.json(
    ...
    const fileName = `staff/${sanitizedStaffId}/avatar_${Date.now()}_${uuidv4().substring(0, 8)}.${fileExt}`;
    ...
    if (uploadError) {
      console.error('❌ [Upload Avatar] Lỗi upload ảnh lên storage:', uploadError.message);
      return NextResponse.json(
    ...
    // Cập nhật luôn avatar_url vào bảng Staff
    await supabase
      .from('Staff')
    ...
    console.error('❌ API Error (Upload Avatar):', error?.message || error);
    return NextResponse.json(
      { success: false, error: 'Lỗi hệ thống khi tải ảnh đại diện.' },
      { status: 500 }
    );
  }
}
